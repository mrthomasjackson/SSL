...
if (isset($_POST['download'])) {
    try {
    	...
    	while ($row = getRow($result)) {
    		...
    		$section->setNoBreak();
    		$section->writeText('<bullet> Price: $' . number_format($row['price'], 2),
                $fontP, $formatP);
    		$section->writeText('<bullet> Mileage: ' . number_format($row['mileage']),
    				$fontP, $formatP);
    		$section->writeText('<bullet> Transmission: ' . $row['transmission'],
    				$fontP, $formatP);
